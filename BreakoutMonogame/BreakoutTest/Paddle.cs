...
namespace BreakoutTest
{
    class Paddle : DrawableSprite
    {
        ...
        {
            this.Location = new Vector2(Game.GraphicsDevice.Viewport.Width / 2.3f, Game.GraphicsDevice.Viewport.Height / 1.1f); 
        }
        ...
        private void UpdateBallCollisionBasedOnPaddleImpactLocation()
        {
            ...
            if ((ball.Location.X > this.Location.X + (this.spriteTexture.Width / 3)) && (ball.Location.X < this.Location.X + (this.spriteTexture.Width / 3) * 2))
            {
                ...
                //50% right or left direction applied so you don't have a constant up and down game
                if(r.Next() % 2 == 0)
                {
                    ball.Direction.X += .1f;
                }
